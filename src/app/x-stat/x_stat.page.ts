...
export class EditStats implements AfterViewInit {
    ...
    async refresh() {
        ...
        for(const attr of attribs){
            if(attr.clusterID === gConst.CLUSTER_ID_MS_TEMPERATURE_MEASUREMENT){
                const thermostat = {} as gIF.thermostat_t;
            ...
            if(attr.clusterServer){
                if(attr.clusterID === gConst.CLUSTER_ID_GEN_ON_OFF){
                    const on_off_actuator = {} as gIF.on_off_actuator_t;
        ...
        await this.storage.delAllThermostat();
        for(const thermostat of this.thermostats){
            await this.storage.storeThermostat(thermostat);
        }
        ...
        if(this.thermostats.length > 0){
            ...
            this.ngZone.run(async ()=>{
                await this.setThermostat(this.selThermostat);
            });
     ...
     *
     */
    async setThermostat(thermostat: gIF.thermostat_t){
     ...
     *
     */
    async thermostatSelected(event: MatSelectChange){
    ...
    public setThermostatDesc(thermostat: gIF.thermostat_t){
        ...
        this.thermostatDesc = [];
        const partDesc: gIF.part_t = this.dlgData.partMap.get(thermostat.partNum);
        if(partDesc){
    ...
    showTooltip(tt: MatTooltip,
        ...
        const partDesc: gIF.part_t = this.dlgData.partMap.get(on_off.partNum);
        if(partDesc){
     ...
     *
     */
    async usedDrop(event: CdkDragDrop<any[]>) {
        ...
        if(event.previousContainer !== event.container){
            ...
            await this.storage.storeThermostat(this.selThermostat);
        }
        else {
     ...
     *
     */
    async freeDrop(event: CdkDragDrop<any[]>) {
    ...
    putEmpties(){
        ...
        for(let i = this.on_off_used.length; i < 6; i++){
            const on_off = {} as gIF.on_off_actuator_t;
            on_off.valid = false;
